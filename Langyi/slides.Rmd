...
## Executive summary
Objective
 - Quantify the financial risk of tidal flooding to address stakeholder concern

Data
 -All properties (>3 million) in South Florida
 -Attributes and historical transaction information
...
*Some codes are partially displayed. For full code I recommend to go to rmd file.
*I have signed a confidentiality agreement with First Street Foundation, and please let me know before you reuse, share, or distribute the materials.
...
Data Science Solution: Quantify the impact by calculating financial loss
 - Supervised learning to target the market value of a property
 - A regression problem to model the fluctuating numerical price

---

## 1. Data preparation
Select relevant features from raw data, which comes from a real estate broker's data base. Observations are properties and variables are relevant attributes.
```{r}
...
risk_dta_original<-fread("D:/raw_data/SF_Parcel_Risk_and_Spatial_Data.csv")
risk_dta<-risk_dta_original%>%select(attomid=ATTOM_ID,
                                     dist_coast,
                                     mdkt32,
                                     Totpopbg:near_reading_rates,
                                     )
save(risk_dta,file = "D:/Langyi/SF_Parcel_Risk_and_Spatial_Data_reduced.RData")
```

```{r, eval=FALSE, echo=FALSE, warning=FALSE}
#Import dataset 3: property transactions, subset and save
trans_dta_original<-fread("D:/raw_data/SF_Sales_Transactions_Data.csv")
trans_dta<-trans_dta_original%>%group_by(attomid)%>%summarize(trans_times=n())
save(trans_dta,file = "D:/Langyi/SF_Sales_Transactions_Data_reduced.RData")
```

```{r, echo=FALSE, warning=FALSE}
#Load saved RData
load("D:/Langyi/Taxassessor_reduced.RData")
load("D:/Langyi/SF_Parcel_Risk_and_Spatial_Data_reduced.RData")
home_dta<-left_join(home_dta,risk_dta,by="attomid")
load("D:/Langyi/SF_Sales_Transactions_Data_reduced.RData")
...
home_dta$ownertypedescription1[home_dta$ownertypedescription1=="NP"|
                                 home_dta$ownertypedescription1=="UNKNOWN"|
                                 home_dta$ownertypedescription1==""|
                                 is.na(home_dta$ownertypedescription1)==TRUE]<-0
home_dta$ownertypedescription2[home_dta$ownertypedescription2=="NP"|
                                 home_dta$ownertypedescription2=="UNKNOWN"|
                                 home_dta$ownertypedescription2==""|
...
---
## 1. Data preparation
Drop unary features at aggregated level
```{r, echo=TRUE, warning=FALSE}
#Drop unary variables in dataset
drop.unary<-function(dta){
  for (i in names(dta)[-1:-14]){
    var<-dta[[i]]
...
screen_names<-home_dta%>%select()
for (i in names(home_dta)[-44:-91]){
  if (i>14){
...
grid.arrange(var_plots[[start_num]],
...
names(county_summary)<-c("NAME",
                         "Number_of_Properties",
                         "Average_Price","Average_Built_Year",
                         "Average_Living_Area")
parcel@data<-inner_join(parcel@data,county_summary,by="NAME")
#save(parcel,file = "D:/Langyi/graph_data/parcel.RData")
```
...
tmap_arrange(summary.county(varname="Number_of_Properties"),
...
## 2. Exploratory analysis
```{r, echo=TRUE, warning=FALSE}
by_year_county<-home_dta%>%group_by(situscounty,deedlastsaledate)%>%summarize(num=n(),
...
#save(by_year_county,file = "D:/Langyi/graph_data/by_year_county.RData")
```
...
## 2. Exploratory analysis
```{r, echo=TRUE, warning=FALSE}
by_year_use<-home_dta[home_dta$propertyusegroup!=0,]%>%group_by(propertyusegroup,deedlastsaledate)%>%summarize(num=n(),
                                                                                           price=median(deedlastsaleprice,na.rm=TRUE),
                                                  area=mean(areabuilding,na.rm=TRUE),
                                              averageprice=mean(deedlastsaleprice/areabuilding,na.rm=TRUE),
                                                  builtyear=mean(yearbuilt,na.rm=TRUE)
...
## 2. Exploratory analysis
```{r, echo=TRUE, warning=FALSE}
by_year_own<-home_dta[home_dta$ownertypedescription1!=0,]%>%group_by(ownertypedescription1,deedlastsaledate)%>%summarize(num=n(),
                                                                                           price=median(deedlastsaleprice,na.rm=TRUE),
                                                  area=mean(areabuilding,na.rm=TRUE),
                                              averageprice=mean(deedlastsaleprice/areabuilding,na.rm=TRUE),
                                                  builtyear=mean(yearbuilt,na.rm=TRUE)
...
home_dta_flood$dist_coast1[home_dta_flood$dist_coast<=med_dist_coast]<-"no flood risk"
home_dta_flood$dist_coast1[home_dta_flood$dist_coast>med_dist_coast]<-"with flood risk"
home_dta_flood$dist_coast1<-home_dta_flood$dist_coast1%>%as.character()
home_dta_flood$floodrisk1<-NA
home_dta_flood$floodrisk1[home_dta_flood$mdkt32<=med_floodrisk]<-"no flood risk"
home_dta_flood$floodrisk1[home_dta_flood$mdkt32>med_floodrisk]<-"with flood risk"
home_dta_flood$floodrisk1<-home_dta_flood$floodrisk1%>%as.character()
by_year_floodrisk<-home_dta_flood%>%group_by(floodrisk1,deedlastsaledate)%>%summarize(num=n(),
                                                                                           price=median(deedlastsaleprice,na.rm=TRUE),
                                                  area=mean(areabuilding,na.rm=TRUE),
                                              averageprice=mean(deedlastsaleprice/areabuilding,na.rm=TRUE),
...
## 2. Exploratory analysis
```{r, echo=TRUE, warning=FALSE}
by_city<-home_dta_flood%>%group_by(propertyaddresscity)%>%summarize(num=n(),
...
by_city$floodrisk1[by_city$floodrisk>med_floodrisk]<-"high flood risk"
...
```{r, echo=TRUE, warning=FALSE}
model.comparison.tree<-function(dta,preprocess_retention=retention){
...
```{r, echo=TRUE, warning=FALSE}
model.comparison.regularization<-function(dta,preprocess_retention=retention){
preprocess_retention<-retention
#Split training/test samples (0.7:0.3)
train<-sample_frac(dta,size=0.7)
test<-anti_join(dta,train,by="attomid")
#Prepare y and x features in train
train<-train%>%select(-attomid,
                  -situsstatecode,
                  -situscounty,
                  -propertyaddresscity)
essential.var<-intersect(names(dta),c("deedlastsaledate",
                       "yearbuilt",
                       "areabuilding",
                       "censustract"))%>%paste(collapse = "+")
if ("deedlastsaledate"%in%names(dta)){
  formula.1<-as.formula(paste("deedlastsaleprice~
                   I(deedlastsaledate^5)+
                  I(deedlastsaledate^4)+
                  I(deedlastsaledate^3)+
                  I(deedlastsaledate^2)+(",
                  paste(essential.var),")^2+
                    (.)"))
}else{
  formula.1<-as.formula(paste("deedlastsaleprice~(",paste(essential.var),")^2+(.)"))
}
...
out <- glmnet(x.train,y.train, nlambda = 1, lambda = bestlam, alpha = 0)
...
bestlam <- cv.out$lambda.min
out <- glmnet(x.train,y.train, nlambda = 1, lambda = bestlam, alpha = 1)
...
LASSO_coef<-coef(out)%>%row.names()%>%cbind(coef(out)%>%as.numeric())
...
LASSO_coef<-LASSO_coef[order(LASSO_coef$abs_coef,decreasing = TRUE),]
...
model_city_regularization<-modeling(home_city_cleaned_dta,"regularization",sz=10000)
model_summary<-inner_join(model_city_tree,
                          model_city_regularization%>%select(subsample,
                                                             ridge_MSE:LASSO_names))
save(model_summary,file = "D:/Langyi/model_summary.RData")
```

---
## 7.5 Visualize model performance
```{r, echo=TRUE, warning=FALSE}
load("D:/Langyi/citylevel_fullsample_tree_regularization.RData")
city_stats<-home_dta%>%group_by(propertyaddresscity)%>%summarise(num=n(),
                                                                 subsample=str_c(situsstatecode[1],
                                                                                 situscounty[1],
...
major_models$rf_MPE<-major_models$rf_MPE%>%as.character()%>%as.numeric()
...
major_models$floodrisk1[major_models$floodrisk>0&
                          major_models$floodrisk<=med_floodrisk1]<-"low flood risk"
major_models$floodrisk1[major_models$floodrisk>med_floodrisk1]<-"high flood risk"
