...
```{r, echo=TRUE, warning=FALSE}
ggplot(data=city_obs)+geom_area(aes(x = city_obs$Properties),stat = "bin",fill = "khaki2")+xlab("Numbers of properties in data")+ylab("Number of cities")+ggtitle("Most cities have few total number of properties")+theme_classic()
```
---
## 3. Seperate modeling: one model for each city/county data
...
The random forest outperforms boosting in most of the cases in cities/counties, across different measure of prediction accuracy. This is reasonable given our data is quite varying and is with a lot of correlated features, and the feature selection in random forest make the process more robust to different data.
---
## 8. Personalize filtering parameters for random forest
